...
export async function transactionsGetTransactions(
  client: JaniPaymentsCore,
  request: operations.GetTransactionsRequest,
  options?: RequestOptions,
): Promise<
  Result<
    operations.GetTransactionsResponseBody,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.GetTransactionsResponseBody$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
